...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for i in range(3):
            l = reader.next()
        # YOUR CODE HERE
        # Extracting 'rdf-schema#label' and 'binomialAuthority_label'
        for line in reader:
...
def update_db(data, db):
    ...
    # YOUR CODE HERE
    for key in data:
        label = db.arachnid.find_one({"label":key})
        label['classification']['binomialAuthority'] = data[key]
        db.arachnid.save(label)
